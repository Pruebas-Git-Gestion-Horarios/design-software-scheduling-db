...
Motor de asignacion de horarios: franjas, sesiones de clase y conflictos.
...
- **Schema(s) propio(s):** `scheduling` — ninguna tabla queda en `public` (aislamiento por microservicio).
...
docker compose --env-file .env.develop --profile tooling run --rm liquibase-scheduling update
```

Estado / rollback:

```bash
docker compose --profile tooling run --rm liquibase-scheduling status --verbose
docker compose --profile tooling run --rm liquibase-scheduling rollbackCount 1
```
